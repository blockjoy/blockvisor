use babel::{
    fetch_blockchain::FetchBlockchain, job_runner::JobRunner, jobs, log_buffer::LogBuffer,
    run_sh::RunSh, BABEL_LOGS_UDS_PATH,
};
...
async fn main() -> eyre::Result<()> {
    ...
    match job_config.job_type {
        ...
        JobType::FetchBlockchain {
            ...
            FetchBlockchain::new(
                bv_utils::timer::SysTimer,
                manifest.ok_or(anyhow!("missing BlockchainDataManifest"))?,
                destination,
                job_config.restart,
            )?
            .run(run, &job_name, &jobs::JOBS_DIR)

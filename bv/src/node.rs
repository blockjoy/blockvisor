...
use babel_api::config::{firewall, JobConfig, RestartConfig, RestartPolicy};
...
impl<P: Pal + Debug> Node<P> {
        ...
        let (pid, node_conn) = match get_process_pid(FC_BIN_NAME, &cmd) {
            Ok(pid) => {
                ...
                .await
                {
                    ...
                    Err(err) => {
                        warn!("failed to reestablished babel connection to running node {node_id}: {err}");
                        NodeConnection::closed(&paths.chroot, node_id)
                    }
                };
                (Some(pid), node_conn)
    ...
    async fn load_bin(babel_path: &Path) -> Result<(Vec<babel_api::Binary>, u32)> {
        let file = File::open(babel_path)
            .await
            .with_context(|| format!("failed to load binary {}", babel_path.display()))?;
